...
class Output:
    def __init__(self, airgap_flux_density=None, flux_linkage=None, back_emf_phase=None, back_emf_line=None, cogging_torque=None, torque_maxwell_stress_tensor=None):
...
def solve(spm, solve_airgap=True, solve_flux_linkage=True, solve_back_emf_phase=True, solve_back_emf_line=True, solve_cogging_torque=True, show_plot=True, debug=True, save_data=True, quality="medium", angle_unit="radian"):
    plot_data = []
    ...
    if quality == "low":
        ...
        n_rotor_yoke, n_magnet, n_airgap, n_tooth_tip, n_tooth, n_stator_yoke = 2,3,5,4,3,2
    elif quality == "high":
        total_col, theta_resolution, step_cogging, step_standard, n_point_cogging, n_point_standard, n_point_check = find_solver_parameter(spm,n_point_plot=33)
        ...
        total_col, theta_resolution, step_cogging, step_standard, n_point_cogging, n_point_standard, n_point_check = find_solver_parameter(spm,n_point_plot=31)
        n_rotor_yoke, n_magnet, n_airgap, n_tooth_tip, n_tooth, n_stator_yoke = 1,1,3,3,2,1
    ...
    airgap_flux_density = np.zeros((4, number_of_cols))
    ...
    with tqdm(range(n_point_check), desc=" Solving", **TQDM_KWARGS) as pbar:
        for i in pbar:
            if i == 0:
                reluctance_network = quasi_newton(reluctance_network,detail_debug=debug)
                for j in range(number_of_cols):
                    airgap_flux_density[-1,j] = j*theta_resolution
                    position = (airgap_position,j)
                    airgap_flux_density[0,j] = reluctance_network.get_element(position).average_flux_density
                    airgap_flux_density[1,j] = reluctance_network.get_element(position).flux_density_radial
                    airgap_flux_density[2,j] = reluctance_network.get_element(position).flux_density_tangential
                coenergy[-1,0] = 0 
                ...
                if i < n_point_cogging:
                    ...
                    if i % step_standard == 0:
                        ...
                        flux_linkage_position = reluctance_network.flux_linkage()*spm.reduce_factor
                        for k in range(number_of_phase): flux_linkage[k,j] = flux_linkage_position[k]
    print_inline("RN calculated:", time.perf_counter()-time_start, "second")
    ...
    return Output(airgap_flux_density,flux_linkage,back_emf_phase,back_emf_line,cogging_torque,torque_maxwell_stress_tensor)

result = solve(spm = spm)